...
const ShopCart = () => {
  ...
  // 총 결제 금액 계산 (체크된 아이템만 포함)
  const totalAmount =
    selectedItems.length > 0
      ? cart
          .filter((item) => selectedItems.includes(item.cartNo))
          .reduce((acc, item) => acc + item.goodsPrice * item.goodsEa, 0)
      : cart.reduce((acc, item) => acc + item.goodsPrice * item.goodsEa, 0); // 체크된 항목이 없을 경우 모든 항목의 총합
  ...
  const handleCheckout = () => {
    ...
    axios
      .post(`${backServer}/goods/cart/checkout`, { items: itemsToPurchase })
      .then((res) => {
        ...
        Swal.fire({
          ...
          navigate("/shop/confirmation"); // 결제 완료 후 정보 페이지로 이동
        });
      })
      .catch((err) => {
